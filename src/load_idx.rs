...
// pub fn load_idx<T: Copy + Debug>(file_name: &str) -> Result<Idx<T>> {
//     let path = Path::new(file_name);
//     println!("loading {}...", path.display());
//     let file = File::open(&path)?;

//     let mut bytes = file.bytes();
//     if !bytes.by_ref().take(2).all(|x| x.unwrap_or(1u8) == 0u8) {
//         return Err(Error::new(
//             ErrorKind::InvalidInput,
//             format!("Invalid file format: {}", file_name),
//         ));
//     };
//     let dtype: u8 = bytes.next().unwrap().unwrap();
...
//     let n_dimensions = bytes.next().unwrap().unwrap();
//     let mut dimensions = vec![];
//     for _ in 0..n_dimensions {
//         dimensions.push(unsafe { from_bytes::<i32>(&mut bytes) as usize });
//     }
...
//     let mut values = vec![];
//     for _ in 0..size {
//         values.push(unsafe { from_bytes::<T>(&mut bytes) });
//     }

//     Ok(Idx { dimensions, values })
// }